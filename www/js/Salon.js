class Salon extends Component {
  ...
  seatHoverEffect(e) {
    ...
    if (a + this.nbrOfPickedSeats < this.totalSeats + 1) {
      for (let i = 0; i < this.nbrOfPickedSeats; i++) {
        ...
        if (this.alreadyBookedSeats.includes(b.seatNum)) {
          continue;
        }
        b.blinkMe = true;
      }
  ...
  toggleSeat(e) {
    ...
    this.lastSeatIndex = this.bookedSeats.indexOf(
      this.mySeatNbr + this.nbrOfPickedSeats - 1
    );
    ...
    let lastAlreadyIndex = this.alreadyBookedSeats.indexOf(
      this.mySeatNbr + this.nbrOfPickedSeats - 1
    );
    ...
    if (
      seatIndex < 0 &&
      this.lastSeatIndex < 0 &&
      this.lastSeatNbr < this.totalSeats &&
      this.mySeatNbr <= outOfSeatsIndex &&
      alreadyIndex < 0 &&
      lastAlreadyIndex < 0 &&
      this.checksIfChosenSeatsAreEmpty()
    ) {
      ...
      for (let i = 0; i < this.nbrOfPickedSeats; i++) {
        ...
        let seat = this.seatsBySeatNumber[
          seats.eq(myIndex + i).attr("data-seat")
        ];
  ...
  async pushOlderBookedSeatsToArray() {
    ...
    for (
      let viewIndex = 0;
      viewIndex < fakeViewingIndexes.length;
      viewIndex++
    ) {
